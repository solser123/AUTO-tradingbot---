...
class ExecutionRouter:
    def __init__(self, exchange: BinanceExchange, registry: OrderRegistry | None = None) -> None:
    ...
    def execute_market_order(self, plan: MarketOrderPlan) -> ExecutionResult:
        ...
        if self.registry is not None:
            lifecycle = self.registry.from_order(
                symbol=plan.symbol,
                side=plan.side,
                order_type="market",
                requested_qty=plan.requested_quantity,
                reduce_only=plan.reduce_only,
                order=resolved_order,
                status=status,
            )
            self.registry.record(lifecycle)
        return ExecutionResult(
            accepted=True,
            symbol=plan.symbol,
            side=plan.side,
